...
import { ArrowLeft, Trophy, Calendar, Users, Gift, Image, CheckCircle2, XCircle, Clock, Target, UsersRound } from "lucide-react";
...
export default function FamilyChallengeDetailPage() {
    ...
    return (
      <div className="space-y-6">
        <div className="text-center py-12 text-red-500">
          <p>Error: {error}</p>
          <Link href="/admin/families/challenges">
  ...
  const getProofStatusBadge = (status) => {
    // For together module, don't show status badges (no approval flow)
    if (challenge?.module === 'together') {
      return null;
    }
    const variants = {
      pending: "secondary",
      ...
      <div className="grid gap-6 md:grid-cols-2">
                    ...
                    <div className="space-y-1">
                      {rewardItems.map((item, index) => (
                        <div key={`reward-item-${index}`} className="flex items-center gap-1">
                          ...
                          <div>
                            Reward {index + 1}= {item?.name || "Unnamed"}
                            {item?.description && (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Participating Members ({challenge.participants?.length || 0})
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <div className="space-y-2">
                {challenge.participants.map((participant, index) => (
                  <div key={participant._id || index} className="flex items-center justify-between p-2 border rounded">
                    <span className="text-sm">
                      {participant.fullName || participant.username || participant.name || "Unknown"}
                    </span>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-sm text-muted-foreground">No members participating</p>
            )}
          </CardContent>
        </Card>
            ...
            <CardContent>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium">
                  {challenge.winnerId?.fullName || challenge.winnerId?.username || challenge.winnerId?.name || "Unknown"}
                </span>
              </div>
            </CardContent>
          </Card>
        ...
        <Card className="md:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              View all proofs submitted by family members
            </CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium text-sm">
                          {proof.participantId?.fullName || proof.participantId?.username || proof.participantId?.name || "Unknown Member"}
                        </p>
                      ...
                      {getProofStatusBadge(proof.status) && (
                        <div>{getProofStatusBadge(proof.status)}</div>
                      )}
                    </div>
